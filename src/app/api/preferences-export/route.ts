...
import { getAllShiftDatePreferencesForExport } from '@/server-actions/shift-date-preference';
...
export async function GET(req: NextRequest) {
  ...
  const preferences = await getAllShiftDatePreferencesForExport(exportParams);

  const userLabel = await getServerTranslation('app', 'user');
  const dateLabel = await getServerTranslation('app', 'date');
  const preferenceTypeLabel = await getServerTranslation(
    'app',
    'preferenceType'
  );
  ...
  const records = preferences.map((preference) => ({
    [userLabel]: getUserDisplayName(preference.user),
    [dateLabel]: new Date(preference.date).toLocaleDateString(),
    [preferenceTypeLabel]: preference.preference_type,
    [createdAtLabel]: new Date(preference.created_at).toLocaleDateString(),
  }));

  const filename = `preferences_${new Date().toISOString().slice(0, 10)}`;
